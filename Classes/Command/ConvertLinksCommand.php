...
use Imhlab\TtnewsLinkConverter\Service\NewsRecordWriter;
...
final class ConvertLinksCommand extends Command
{
    public function __construct(
        ...
        private readonly NewsRecordWriter $writer,
    ...
    {
        $this
            ->setDescription('Convert legacy tt_news RTE link tags in tx_news bodytext to TYPO3 record-link syntax')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->table(
            ...
            [
                ...
                ['Rows with changes', (string)count($changed)],
        ...
        if ($execute && $changed !== []) {
            $writeResult = $this->writer->updateBodytext(
                array_combine(
                    array_map(static fn (ConversionResult $result): int => $result->sourceUid, $changed),
                    array_map(static fn (ConversionResult $result): string => $result->newBodytext, $changed),
                )
            );
        ...
        if ($execute) {
            $io->success(sprintf(
                'Executed: updated %d row(s), %d link(s) converted. %d issue(s) logged to %s.',
                count($changed),
                $convertedLinkCount,
                count($issues),
                $logFile
            ...
            $io->success(sprintf(
                'Dry-run: %d row(s) would change, %d link(s) would convert. %d issue(s) logged to %s.',
                count($changed),
                $convertedLinkCount,
                count($issues),
                $logFile
